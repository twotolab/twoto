package {
	...
	import com.twoto.gridScanner.DrawWithAllLineElement;
	...
	[SWF(backgroundColor='0xffffff', width='1200', height='1000', frameRate="60")]
	...
	public class AllMultilineClock extends AbstractContent implements IContent {
		//---------------------------------------------------------------------------
		// 	private variables
		//---------------------------------------------------------------------------
		...
		private var decimalHoursElement:DrawWithAllLineElement;
		private var posXdecimalHoursElement:uint;
		private var baseHoursElement:DrawWithAllLineElement;
		private var posXbaseHoursElement:uint;
		private var decimalMinutesElement:DrawWithAllLineElement;
		private var posXDecimalMinutesElement:uint;
		private var baseMinutesElement:DrawWithAllLineElement;
		private var posXbaseMinutesElement:uint;
		private var doubleDotsElement:DrawWithAllLineElement;
		...
		// 	constructor
		//---------------------------------------------------------------------------
		public function AllMultilineClock() {

			colorChoice=0xED2368;
			backGroundColor=0x38001A;//55002A;
			backGroundShadowColor=0x38001A;
			addedSpace=20;
			maxTextWidth=828;
			letterWidth=166;
			letterHeight=362;
		...
		private function init():void {
			...
			descriptionText=simpleText("", new Standard_55_Font() as Font, colorChoice, 8);

			descriptionText.text=TEXT_SCANNING;
			addChild(descriptionText);

			lineCounterText=TextUtils.simpleText(".", new Standard_55_Font(), colorChoice, 8);
...
if(clock != null){
			...
			decimalHoursElement=new DrawWithAllLineElement(generatedText(clock.decimalHours.toString(), posXdecimalHoursElement), colorChoice, centerPt);
...
if(clock != null){
			...
			var addedPlusOne:uint=(clock.decimalHours.toString() == "1") ? addedSpace * 1.5 : 0;
			baseHoursElement=new DrawWithAllLineElement(generatedText(clock.baseHours.toString(), posXbaseHoursElement), colorChoice, centerPt);
...
if(clock != null){
			...
			doubleDotsElement=new DrawWithAllLineElement(generatedText(":", posXdoubleDotsElement), colorChoice, centerPt);
...
if(clock != null){
			...
			var addedPlusOne:uint=(clock.decimalHours.toString() == "1") ? addedSpace * 1.5 : 0;
			decimalMinutesElement=new DrawWithAllLineElement(generatedText(clock.decimalMinutes.toString(), posXDecimalMinutesElement), colorChoice, centerPt);
...
if(clock != null){
			...
			var addedPlusOne:uint=(clock.decimalHours.toString() == "1") ? addedSpace * 1.5 : 0;
			baseMinutesElement=new DrawWithAllLineElement(generatedText(clock.baseMinutes.toString(), posXbaseMinutesElement), colorChoice, centerPt);
