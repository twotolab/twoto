package com.twoto.utils.scroller {
	...
	public class TimeLineScroller {
		...
		private function updateTimelinePosition(evt:Event):void {
			...
			if (accelerationX < addedXValue){
				accelerationX+=friction;
			}
			else if (accelerationX > addedXValue){
				accelerationX-=friction;
			}
			else if (accelerationX == addedXValue){
				accelerationX=0;
			...
			if (timeLineContainer.x <= x || timeLineContainer.x <= maxContainerPos) {
				...
				if (Math.abs(accelerationX) > 0.5){
					timeLineContainer.x-=accelerationX;
