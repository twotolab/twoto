package
{
	...
	public class Molecule_blue extends AbstractCellsClass implements IContent
	{
		...
		private var colorChoice:uint =0x00E9E9;
		
		public function Molecule_blue()
		{
		...
		private function randomSpritesVelocity():void{
			...
			for ( i=0; i<sprites.length; i++) {
				...
				if( Math.abs(sprite.vx)>0.01 || Math.abs( sprite.vy)>0.01){
					...
					if(hitTest(sprite) == true && sprite.over != true){
						sprite.overCircle(colorChoice);//0xFFFF00//0x00E9E9
		...
		private function setFriction(partA:Ball):void{
			//var friction:Number=(Math.abs(partA.vx)<0.001)?0.01:0.02;
